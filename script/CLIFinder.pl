...
sub save_csv{
  ...
  eval
  {
    $R->run(qq`
      ...
      grchim <- GRanges(seqnames = chim[,1], IRanges(start = chim[,2], end = chim[,3]),strand=chim[,4])
      ...
      grchim2 <- GRanges(seqnames = datax[,$#fastq1+11], IRanges(start = datax[,$#fastq1+12], end = datax[,$#fastq1+13]),strand=datax[,$#fastq1+14])
      ...
      grchim <- GRanges(seqnames = result\$L1.Chr, IRanges(start = result\$L1.Start, end = result\$L1.End),strand=result\$L1.Strand)
      mcols(grchim)<-result
      Rep<-read.delim("$Line_only",skip=1)
      Gene<-read.delim("$Hg19_refseq")
      grLINE <- GRanges(seqnames = Rep\$genoName, IRanges(start = Rep\$genoStart, end = Rep\$genoEnd), repStrand=as.character(Rep\$strand), repName =as.character(Rep\$repName))
      grGene <- GRanges(seqnames = Gene\$chrom, IRanges(start = Gene\$txStart, end = Gene\$txEnd), geneStrand=as.character(Gene\$strand), geneName = as.character(Gene\$name2))
